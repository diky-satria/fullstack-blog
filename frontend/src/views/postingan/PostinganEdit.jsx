...
export default function PostinganEdit() {
  const { id } = useParams();
  const navigate = useNavigate();

  const [judul, setJudul] = useState("");
  const [judulLama, setJudulLama] = useState("");
  ...
  const getDetail = async () => {
    try {
      ...
      setJudulLama(response.data.data.judul);
      setTechStack(response.data.data.techstack);
      setDeskripsi(response.data.data.deskripsi);
      setThumbnailShow(response.data.data.thumbnail);
  ...
  const selanjutnya = async (page) => {
    ...
    if (page === 0) {
      try {
        ...
        formData.append("judul_lama", judulLama);
        formData.append("techstack", techStack);
        formData.append("thumbnail", thumbnail);
        formData.append("deskripsi", deskripsi);

        await axios.post("/api/post/editOne", formData, {
    ...
    if (page === 1) {
      try {
        await axios.post("/api/post/editTwo", {
          content: content,
        });
        setPage((currPage) => currPage + 1);
        setLoading(false);
            ...
            <div className="card mt-3">
              {page === 0 ? (
                <div className="card-body">
                  <h5 className="mb-4">Edit Postingan</h5>
                  ...
                  <div className="form-group mt-3">
                    ...
                    <Form key={thumbnail}>
                      <Input type="file" id="thum" onChange={loadthumbnail} />
                    </Form>
